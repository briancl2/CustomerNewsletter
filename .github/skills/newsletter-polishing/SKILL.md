...
| 20 | **Risky claim source audit** | Billing, pricing, model availability, legal/preview, CCC/DPA, BYOK, OpenTelemetry, and cost-optimization claims require current source support. Use first-party GitHub/Microsoft sources for Copilot product and billing behavior; provider docs may support only general token-economics context. |
| 21 | **Evidence-label audit** | Optimization claims must label the evidence type: official behavior, GitHub usage report/export, internal telemetry, workflow experiment, anecdote, recommendation, or inference. Remove or hedge any claim that implies billing proof, durable savings, model superiority, or fleet readiness without evidence. |
| 22 | **Action-owner audit** | Each major section should include a recommended action, owner cue, or explicit `awareness only` treatment. This is mandatory for lead sections, billing/governance sections, legal/preview notes, and high-risk migration/deprecation content. |
| 23 | **BYOK scope audit** | Replace generic `BYOK token usage` wording with the specific surface and billing/reporting context: VS Code BYOK, Copilot CLI external/local provider, SDK/custom workflow, direct provider billing, model availability, or GitHub AI Credits impact. |
| 24 | **OpenTelemetry scope audit** | Scope OpenTelemetry to the actual instrumented surface. For SDK-backed workflows, cite Copilot SDK/OpenTelemetry. For standard VS Code or Copilot CLI usage, prefer GitHub usage reports, dashboards, and available IDE/CLI telemetry unless the source explicitly documents traces. |
| 25 | **Developer billing caveat audit** | When UBB or AI Credits affect developer behavior, confirm whether the copy needs the code-completions/Next Edit Suggestions caveat and Copilot code review caveat. Code review may involve AI Credits and GitHub Actions minutes; completions/NES are outside AI Credit billing for paid plans. |
